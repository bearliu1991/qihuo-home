<template>
  <div id="good_img_upload">
    <el-upload
      ...
      <!-- :limit="maxImgNum" -->
      <ul class="imgs_list">
        <li v-for="(item, idx) in imgTotalList" :item="item" class="slick_item" :index="idx" :key="idx">
          <div class="item_wrap">
    ...
    <el-dialog
        ...
        <div style="text-align:center">
          <img style="max-height:90%; max-width:90%" :src="imgTotalList[bigImgIdx]">
        </div>
      </el-dialog>
  </div>
</template>
<script>
  export default {
    ...
    methods: {
      ...
      moveNext(idx, isNext) {
        if (isNext) {
          let obj = this.imgTotalList[idx + 1]
      ...
      getParams() {
        let arr = []
        if (this.isGroup) {
          let picList = this.imgTotalList.slice(1).forEach((item, index) => {
            item && arr.push({position: index, url: item})
          })
          return {imgUrl_main: this.imgTotalList[0], picList: arr}
        } else {
          return this.imgTotalList
        }
      },
      clearImg() {
        this.fileList = []
        ...
        if (res.success) {
          ...
          fileList.forEach((item, index) => {
            ...
            if (this.processIdx + index < 5) {
              item.response && this.$set(this.imgTotalList, this.processIdx + index, item.response.data)
            }
          })
          this.$emit('uploadImg', this.getParams())
          if (allTrue) {

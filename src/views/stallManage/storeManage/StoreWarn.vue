  ...
  <div id="store_warn">
    <el-table
      ...
      <el-table-column prop="quantity" min-width="100px">
        <template slot="header" slot-scope="scope">
          <div class="header_title">现有库存</div>
          <div class="header_cont">
        ...
        <template slot-scope="scope">
          <div style="text-align: left" :class="[storeType === 'warn' && scope.row.isStoreOk === false && 'err_color bold']">{{scope.row.quantity}}</div>
      ...
      <el-table-column prop="quantity" min-width="100px">
        <template slot="header" slot-scope="scope">
          <div class="header_title">{{columnTitle}}</div>
          <div class="header_cont">
            <el-input size="mini" v-model.trim.number="groupAdd.stockWarn" :placeholder="columnTitle"/>
            <el-button type="primary" plain size="mini" @click='fillInStore(true)'>批量填充</el-button>
          </div>
        </template>
        <template slot-scope="scope">
          <el-input size="mini" v-model='scope.row.stockWarn' @blur="fillInStore(false, scope.row)"></el-input>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.columnTitle = this.storeType === 'inventory' ? '盘点库存' 
        : this.storeType === 'add' ? '新增入库' : this.storeType === 'warn' ? '库存预警' : ''
    },
    components: {},
    methods: {
      init(temArr) {
        ...
        if (temArr && temArr.length) {
          for (let item of temArr) {
            for (let n of item.skuVoList) {
              n.oldQuantity = n.stockWarn
              this.getWarnColor(n)
              if (this.storeType === 'inventory' || this.storeType === 'add') { n.stockWarn = '' }
              arr.push(n)
            }
          }
        }
      ...
      calcQuantity(arr) {
        let quantity = 0
        arr.forEach(item => {
          item && this._isInteger(item.quantity) && (quantity += item.quantity - 0)
        })
        return quantity
      },
      getTable() {
      ...
      getParams() {
        ...
        let allEmpty = true
        arr.forEach(item => {
          (this._isInteger(item.stockWarn)) && (allEmpty = false)
          this.getWarnColor(item)
        })
        // 新增库存数据不可全为空
        if (this.storeType === 'add' && allEmpty) {
          this.$message.error(`${this.columnTitle}数据不能全为空`)
          return false
        }
        ...
        //   return false
        // }
        if (this.storeType === 'add' || this.storeType === 'inventory') {
          arr = arr.map(item => {
            return {id: item.id, stockWarn: item.stockWarn}
          })
          return arr.filter(item => this._isHasVal(item.stockWarn) && this._isInteger(item.stockWarn))
        }
        
        if (this.storeType === 'warn') {
          arr = arr.filter(item => item.stockWarn !== item.oldQuantity)
          arr = arr.map(item => {
            return {id: item.id, stockWarn: item.stockWarn}
          })
          return arr
      ...
      fillInStore(isBtn, row) {
        // 判断输入是否为正整数
        let temObj = isBtn ? this.groupAdd : row
        if (!this._isHasVal(temObj.stockWarn)) return false
        if (this.storeType !== 'add' && !this._nonnegativeInt(temObj.stockWarn)) {
          this.$set(temObj, 'stockWarn', '')
          this.$message.error('库存需为非负整数')
          this.getWarnColor(temObj)
          return false
        }
        if (this.storeType === 'add' && !this.isPositiveInt(temObj.stockWarn)) {
          this.$set(temObj, 'stockWarn', '')
          this.$message.error('库存需为正整数')
          this.getWarnColor(temObj)
          return false
        }

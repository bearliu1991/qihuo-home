    ...
    <section class="app-container" id="workList">
      <div v-show="!secShow">
          ...
          <el-form :inline="true" :model="filters" @submit.native.prevent>
            ...
            <el-form-item label="销售时间">
              <date-pick ref="dateFilter" isLimit :maxDays="92" 
                size="mini" defaultIndex="1" :showArr="['today', 'lastWeek', 'last30']"></date-pick>
            </el-form-item>
            <el-form-item>
        ...
        <el-table :data="users" highlight-current-row style="width: 100%;">
          ...
          <el-table-column prop="createTime" min-width="200px" label="创建时间" :formatter="formatTime">
          </el-table-column>
          ...
          <el-table-column prop="salesAmount" min-width="200px" label="销售金额(不包含减免金额)">
            <template slot-scope="scope">
              <span class="bold err_color">{{scope.row.salesAmount}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="returnAmount" min-width="200px" label="退货金额(不包含减免金额)">
          </el-table-column>
        </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      getUsers() {
        ...
        this.$postJson(this._basePath.ajaxNewGoodSaleByPage, {...para, ...this.filters}).then(res => {
          if (res.success) {

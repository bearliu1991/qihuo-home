    ...
    <div v-show="!secShow">
      <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
        <el-form :inline="true" :model="filters" @submit.native.prevent>
          <el-form-item label="VIP客户名">
            <el-input v-model="filters.vipUserName" placeholder="VIP客户名"></el-input>
          </el-form-item>
          <el-form-item label="销售时间">
            <date-pick ref="dateFilter" isLimit 
      ...
      <el-table :data="users" highlight-current-row style="width: 100%;">
        <el-table-column prop="vipUserName" label="VIP客户名">
          <template slot-scope="scope">
            {{scope.row.vipUserName || '—'}}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="salesTotalFee" width="200px" label="销售金额(不包含减免金额)">
          <template slot-scope="scope">
            <span class="bold err_color">{{scope.row.salesTotalFee}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="returnTotalFee" width="200px" label="退货金额(不包含减免金额)">
        </el-table-column>
      </el-table>
  ...
  methods: {
    ...
    getUsers() {
      let para = {
        ...
        vipUserName: this.filters.vipUserName
      }
      ...
      this.$postJson(this._basePath.ajaxSingleGoodVipBuy, {...para, ...this.filters}).then(res => {
        if (res.success) {
          this.total = res.data && res.data.count ? res.data.count : 0
          this.users = res.data && res.data.datas ? res.data.datas : []
        }
      })
    }
  }
